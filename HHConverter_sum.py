	...
	def processLine(self, HH):
		...
		if (self.sidePotMode):
			...
			if (HH.find("Hand #") > -1):
				...
				if (self.SummaryPath == ""):
					try:
						pdata = {'tname': str(self.TOURNEYNAME), 'tdate': str(self.YEAR) + "/" + str(self.MONTH) + "/" + str(self.DAY) + " " + str(self.TIME) + " EST", "api_key":self.api_key }
						data = urlencode(pdata)
						response = urlopen('http://www.bitcoinpokerblog.com/summary.php')
						tsummary = response.read().decode('utf-8')
						print(tsummary)
					except:
						print ("error getting summary")
						tsummary = "error getting summary from BPB ... contact for api key"
						
					self.SummaryPath = self.dest_folder + "FT" + str(self.YEAR) + str(self.MONTH) + str(self.DAY) + " " + str(self.BUYIN) + " " + str(self.TOURNEYNAME) + " - (" + str(self.TOURNEYID) + ") - Summary.txt"
					summaryfile = open(self.SummaryPath, "w")
					summaryfile.write(tsummary)
					summaryfile.close()
